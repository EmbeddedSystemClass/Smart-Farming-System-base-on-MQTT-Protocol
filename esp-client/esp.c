...
const char* password =    "youaresuperhero";
...
void handlerInterrupt()        
{
  ...
  if(BTN1_Status==0 && BTN2_Status==0){
  ...
  else if(BTN1_Status==1 && BTN2_Status==1){
...
{
  TimerCounter++;
  if (TimerCounter == 240){         //240 x 0.5s = 2min
    FlagMQTT=1;
    TimerCounter=0;
  }
}
...
{  
  ...
  static String str_date = String(timeClient.getFormattedDate());
  int splitT = str_date.indexOf("T");
  str_date = str_date.substring(0, splitT);
  static char current_date[50];
  str_date.toCharArray(current_date, 50);
  
  String str_time = String(timeClient.getFormattedTime());
  static char current_time[50];
  str_time.toCharArray(current_time, 50);
  */
  //==========================================================================================
